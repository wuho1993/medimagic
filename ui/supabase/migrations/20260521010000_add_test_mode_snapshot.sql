...
create table if not exists public.test_mode_snapshots (
  session_id uuid not null references public.test_mode_sessions(id) on delete cascade,
  schema_name text not null,
  table_name text not null,
  rows jsonb not null default '[]'::jsonb,
  row_count integer not null default 0,
  primary key (session_id, schema_name, table_name)
);

create or replace function public.start_test_mode_snapshot(
  p_user_id uuid,
...
declare
  ...
  v_row_count integer;
begin
  insert into public.test_mode_sessions (created_by, created_by_email)
  ...
  for v_table in
    ...
    where schemaname = 'public'
      and tablename not like 'test_mode_%'
      and tablename not in ('schema_migrations', 'supabase_migrations')
    order by tablename
  loop
    execute format('select coalesce(jsonb_agg(to_jsonb(t)), ''[]''::jsonb), count(*)::integer from %I.%I t', v_table.schemaname, v_table.tablename)
      into v_rows, v_row_count;

    insert into public.test_mode_snapshots (session_id, schema_name, table_name, rows, row_count)
    values (v_session_id, v_table.schemaname, v_table.tablename, v_rows, coalesce(v_row_count, 0));
  end loop;

  update public.test_mode_sessions
  set table_count = (select count(*) from public.test_mode_snapshots where session_id = v_session_id),
      row_count = (select coalesce(sum(row_count), 0) from public.test_mode_snapshots where session_id = v_session_id)
...
grant execute on function public.start_test_mode_snapshot(uuid, text) to authenticated, service_role;
