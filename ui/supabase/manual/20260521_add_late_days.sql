...
-- Add late_days column to the base table
alter table public.monthly_attendance_records
  add column if not exists late_days numeric default 0;

comment on column public.monthly_attendance_records.late_days is
  'Number of late days in the month. When > 0, attendance bonus should be auto-disabled in payroll.';

-- Rebuild views to include late_days
...
with recursive ordered_attendance as (
  select
    ...
    coalesce(mar.late_days, 0)::numeric(12,2) as late_days,
...
comment on view public.attendance_management_records is
  'Attendance management view with late_days, employee profile payroll basis, no-pay days, deduction base, deduction amount, prorated package commission, and OT accumulation roll-forward rules from 2026-04/2026-05 onward.';
...
comment on view public.payroll_attendance_records is
  'Payroll attendance calculation view with late_days: attendance-driven worked days, no-pay deduction base, scaled base salary/allowances/briefing/attendance/booking bonuses, residual deduction amount, package commission handling, and effective commission amount';
