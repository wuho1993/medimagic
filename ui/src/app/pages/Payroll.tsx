...
${tablePreview}`;
  ...
  const payslipExportEntries = rows.map((row): PayslipPdfEntry => {
    ...
    return {
      ...
      shopCommissionLabel,
      shopActualSalesAmount: row.shopActualSalesAmount,
      redeemVolume: Number(vol.redeem) || 0,
      salesVolume: Number(vol.sales) || 0,
      sgmVolume: Number(vol.sgm) || 0,
  ...
  const handleExportPayrollExcel = async () => {
    ...
    const sumPayrollExcelColumn = (key: string) => filteredRows.reduce((sum, row) => sum + Number((row as Record<string, unknown>)[key] ?? 0), 0);
    ...
    try {
      await saveRowsAsExcel(
        ...
        [
          ...
          { key: 'position', header: '職位', width: 18 },
          { key: 'employmentStatus', header: '僱用狀態', width: 14 },
          { key: 'salaryType', header: '薪酬類型', width: 14 },
          { key: 'payDayPrimary', header: '主要出糧日', width: 12 },
          { key: 'payDaySecondary', header: '第二出糧日', width: 12 },
          { key: 'workedDays', header: '工作日數', width: 12 },
          { key: 'workedHours', header: '工作時數', width: 12 },
          { key: 'lateDays', header: '遲到日數', width: 12 },
          { key: 'sickLeaveDays', header: '病假日數', width: 12 },
          { key: 'sickNoPayDays', header: '病假無薪日數', width: 14 },
          { key: 'noPayDays', header: 'No Pay 日數', width: 12 },
          { key: 'annualLeaveDays', header: 'AL 日數', width: 12 },
          { key: 'statutoryHolidayDays', header: 'SH 日數', width: 12 },
          { key: 'rawBaseSalary', header: '原本底薪', width: 14 },
          { key: 'baseSalary', header: t.cols.base, width: 14 },
          { key: 'allowance', header: '津貼', width: 14 },
          { key: 'transportAllowance', header: '交通津貼', width: 14 },
          { key: 'briefingBonus', header: 'Briefing Bonus', width: 16 },
          { key: 'attendanceBonus', header: '勤工獎', width: 14 },
          { key: 'bookingBonus', header: 'Booking Bonus', width: 16 },
          { key: 'officeJobAmount', header: 'Office Job', width: 14 },
          { key: 'manualBonus', header: '手動加項', width: 14 },
          { key: 'manualBonusRemarks', header: '手動加項備註', width: 24 },
          { key: 'manualDeduction', header: '手動扣減/扣款', width: 16 },
          { key: 'manualDeductionRemarks', header: '扣減備註', width: 24 },
          { key: 'shopTargetAmount', header: '店舖目標', width: 14 },
          { key: 'shopActualSalesAmount', header: '店舖實際銷售', width: 16 },
          { key: 'shopTargetPercent', header: '達標率', width: 12 },
          { key: 'shopBonus', header: '店舖獎金', width: 14 },
          { key: 'redeemVolume', header: 'Redeem 數量', width: 14 },
          { key: 'redeemCommission', header: 'Redeem 佣金', width: 14 },
          { key: 'salesVolume', header: 'Sales 數量', width: 14 },
          { key: 'salesCommission', header: 'Sales 佣金', width: 14 },
          { key: 'salesAmountTotal', header: '銷售額', width: 14 },
          { key: 'salesAmountCommission', header: '銷售額佣金', width: 14 },
          { key: 'jobCommission', header: 'Job 佣金', width: 14 },
          { key: 'sgmVolume', header: 'SGM 數量', width: 14 },
          { key: 'sgmCommission', header: 'SGM 佣金', width: 14 },
          { key: 'streetPromoterHeadcount', header: '街霸人頭', width: 14 },
          { key: 'streetPromoterCommission', header: '街霸佣金', width: 14 },
          { key: 'telesalesHeadcount', header: '電話銷售人頭', width: 14 },
          { key: 'telesalesCommission', header: '電話銷售佣金', width: 16 },
          { key: 'salesBonus', header: 'Sales Bonus', width: 14 },
          { key: 'payrollBonus', header: 'Payroll Bonus', width: 16 },
          { key: 'packageCommissionAmount', header: '包佣金額', width: 14 },
          { key: 'packageCoveredCommission', header: '包佣覆蓋佣金', width: 16 },
          { key: 'packageCommission', header: '最終包佣/保底', width: 16 },
          { key: 'displayedCommission', header: t.cols.commission, width: 14 },
          { key: 'rollingAverageCommission', header: '12個月平均佣金', width: 18 },
          { key: 'annualLeaveAverageCommissionPay', header: 'AL 平均工資', width: 16 },
          { key: 'statutoryHolidayAverageCommissionPay', header: 'SH 平均工資', width: 16 },
          { key: 'alShAverageCommissionPay', header: 'AL/SH 平均工資合計', width: 20 },
          { key: 'attendanceExtraBasePay', header: '額外底薪補薪', width: 16 },
          { key: 'legalMinimumAlShTopUp', header: '法定最低補差額', width: 16 },
          { key: 'alShComplianceWarning', header: 'AL/SH 提示', width: 30 },
          { key: 'grossBase', header: '7th Gross Base', width: 16 },
          { key: 'primaryPayoutGross', header: '7th Gross', width: 16 },
          { key: 'primaryMpf', header: '7th MPF 扣款', width: 16 },
          { key: 'primaryPayoutNet', header: '7th Net', width: 16 },
          { key: 'secondaryPayoutGross', header: '20th Gross', width: 16 },
          { key: 'secondaryMpf', header: '20th MPF 扣款', width: 16 },
          { key: 'secondaryPayoutNet', header: '20th Net', width: 16 },
          { key: 'monthEndMpf', header: '月尾一次扣 MPF', width: 16 },
          { key: 'mpfRelevantIncome', header: 'MPF 相關收入', width: 16 },
          { key: 'calculatedMpf', header: '系統計算 MPF', width: 16 },
          ...
          { key: 'net', header: t.cols.net, width: 16 },
        ],
        filteredRows.map((row) => ({
          ...
          position: row.positionNameZh || '-',
          employmentStatus: payrollStatusLabel(row.employmentStatus),
          salaryType: row.salaryType || row.employmentType || '-',
          payDayPrimary: row.payDayPrimary,
          payDaySecondary: row.payDaySecondary,
          ...
          statutoryHolidayDays: row.statutoryHolidayDays,
          rawBaseSalary: row.rawCalculatedBaseSalary,
          baseSalary: row.calculatedBaseSalary,
          ...
          manualDeduction: row.totalDeduction,
          manualDeductionRemarks: row.manualDeductionRemarks || '',
          shopTargetAmount: row.shopTargetAmount,
          shopActualSalesAmount: row.shopActualSalesAmount,
          shopTargetPercent: row.shopTargetPercent,
          shopBonus: row.shopBonus,
          redeemVolume: row.redeemVolume,
          redeemCommission: row.commissionCalculationEnabled ? row.commResult.redeem.amount : 0,
          salesVolume: row.salesVolume,
          salesCommission: row.commissionCalculationEnabled ? row.commResult.sales.amount : 0,
          salesAmountTotal: row.commResult.salesAmount.total,
          salesAmountCommission: row.commissionCalculationEnabled ? row.commResult.salesAmount.amount : 0,
          jobCommission: row.commissionCalculationEnabled ? row.commResult.job : 0,
          sgmVolume: row.sgmVolume,
          sgmCommission: row.commissionCalculationEnabled ? row.commResult.sgm.amount : 0,
          streetPromoterHeadcount: row.streetPromoterHeadcount,
          streetPromoterCommission: row.streetPromoterCommission,
          telesalesHeadcount: row.telesalesHeadcount,
          telesalesCommission: row.telesalesCommission,
          salesBonus: row.commissionCalculationEnabled ? row.commResult.salesBonus : 0,
          payrollBonus: row.commissionCalculationEnabled ? row.commResult.payrollBonus : 0,
          packageCommissionAmount: row.packageCommissionAmount,
          packageCoveredCommission: row.packageCoveredCommission,
          packageCommission: row.packageCommission,
          displayedCommission: row.displayedCommission,
          rollingAverageCommission: row.rollingAverageCommission,
          annualLeaveAverageCommissionPay: row.annualLeaveAverageCommissionPay,
          statutoryHolidayAverageCommissionPay: row.statutoryHolidayAverageCommissionPay,
          alShAverageCommissionPay: row.alShAverageCommissionPay,
          ...
          secondaryPayoutNet: row.secondaryPayoutNet,
          monthEndMpf: row.monthEndMpf,
          mpfRelevantIncome: row.mpfRelevantIncome,
          calculatedMpf: row.calculatedMpf,
          mpfEe: row.mpfEe,
          mpfEr: row.mpfEr,
        ...
        })),
        {
          ...
          workedDays: sumPayrollExcelColumn('workedDays'),
          workedHours: sumPayrollExcelColumn('workedHours'),
          lateDays: sumPayrollExcelColumn('lateDays'),
          sickLeaveDays: sumPayrollExcelColumn('sickLeaveDays'),
          sickNoPayDays: sumPayrollExcelColumn('sickNoPayDays'),
          noPayDays: sumPayrollExcelColumn('attendanceNoPayDays'),
          annualLeaveDays: sumPayrollExcelColumn('annualLeaveDays'),
          statutoryHolidayDays: sumPayrollExcelColumn('statutoryHolidayDays'),
          rawBaseSalary: sumPayrollExcelColumn('rawCalculatedBaseSalary'),
          baseSalary: sumPayrollExcelColumn('calculatedBaseSalary'),
          allowance: sumPayrollExcelColumn('allowanceAmount'),
          transportAllowance: sumPayrollExcelColumn('transportAllowance'),
          briefingBonus: sumPayrollExcelColumn('briefingBonus'),
          attendanceBonus: sumPayrollExcelColumn('attendanceBonus'),
          bookingBonus: sumPayrollExcelColumn('bookingBonus'),
          officeJobAmount: sumPayrollExcelColumn('officeJobAmount'),
          manualBonus: sumPayrollExcelColumn('manualBonus'),
          manualBonusRemarks: null,
          manualDeduction: sumPayrollExcelColumn('totalDeduction'),
          manualDeductionRemarks: null,
          shopTargetAmount: sumPayrollExcelColumn('shopTargetAmount'),
          shopActualSalesAmount: sumPayrollExcelColumn('shopActualSalesAmount'),
          shopTargetPercent: null,
          shopBonus: sumPayrollExcelColumn('shopBonus'),
          redeemVolume: sumPayrollExcelColumn('redeemVolume'),
          redeemCommission: filteredRows.reduce((sum, row) => sum + (row.commissionCalculationEnabled ? row.commResult.redeem.amount : 0), 0),
          salesVolume: sumPayrollExcelColumn('salesVolume'),
          salesCommission: filteredRows.reduce((sum, row) => sum + (row.commissionCalculationEnabled ? row.commResult.sales.amount : 0), 0),
          salesAmountTotal: filteredRows.reduce((sum, row) => sum + row.commResult.salesAmount.total, 0),
          salesAmountCommission: filteredRows.reduce((sum, row) => sum + (row.commissionCalculationEnabled ? row.commResult.salesAmount.amount : 0), 0),
          jobCommission: filteredRows.reduce((sum, row) => sum + (row.commissionCalculationEnabled ? row.commResult.job : 0), 0),
          sgmVolume: sumPayrollExcelColumn('sgmVolume'),
          sgmCommission: filteredRows.reduce((sum, row) => sum + (row.commissionCalculationEnabled ? row.commResult.sgm.amount : 0), 0),
          streetPromoterHeadcount: sumPayrollExcelColumn('streetPromoterHeadcount'),
          streetPromoterCommission: sumPayrollExcelColumn('streetPromoterCommission'),
          telesalesHeadcount: sumPayrollExcelColumn('telesalesHeadcount'),
          telesalesCommission: sumPayrollExcelColumn('telesalesCommission'),
          salesBonus: filteredRows.reduce((sum, row) => sum + (row.commissionCalculationEnabled ? row.commResult.salesBonus : 0), 0),
          payrollBonus: filteredRows.reduce((sum, row) => sum + (row.commissionCalculationEnabled ? row.commResult.payrollBonus : 0), 0),
          packageCommissionAmount: sumPayrollExcelColumn('packageCommissionAmount'),
          packageCoveredCommission: sumPayrollExcelColumn('packageCoveredCommission'),
          packageCommission: sumPayrollExcelColumn('packageCommission'),
          displayedCommission: sumPayrollExcelColumn('displayedCommission'),
          rollingAverageCommission: null,
          annualLeaveAverageCommissionPay: sumPayrollExcelColumn('annualLeaveAverageCommissionPay'),
          statutoryHolidayAverageCommissionPay: sumPayrollExcelColumn('statutoryHolidayAverageCommissionPay'),
          alShAverageCommissionPay: sumPayrollExcelColumn('alShAverageCommissionPay'),
          attendanceExtraBasePay: sumPayrollExcelColumn('attendanceExtraBasePay'),
          legalMinimumAlShTopUp: sumPayrollExcelColumn('legalMinimumAlShTopUp'),
          alShComplianceWarning: null,
          grossBase: sumPayrollExcelColumn('grossBase'),
          primaryPayoutGross: sumPayrollExcelColumn('primaryPayoutGross'),
          primaryMpf: sumPayrollExcelColumn('primaryMpf'),
          primaryPayoutNet: sumPayrollExcelColumn('primaryPayoutNet'),
          secondaryPayoutGross: sumPayrollExcelColumn('secondaryPayoutGross'),
          secondaryMpf: sumPayrollExcelColumn('secondaryMpf'),
          secondaryPayoutNet: sumPayrollExcelColumn('secondaryPayoutNet'),
          monthEndMpf: sumPayrollExcelColumn('monthEndMpf'),
          mpfRelevantIncome: sumPayrollExcelColumn('mpfRelevantIncome'),
          calculatedMpf: sumPayrollExcelColumn('calculatedMpf'),
          mpfEe: sumPayrollExcelColumn('mpfEe'),
          mpfEr: sumPayrollExcelColumn('mpfEr'),
          pay7th: sumPayrollExcelColumn('primaryPayoutNet'),
          pay20th: sumPayrollExcelColumn('secondaryPayoutNet'),
          net: sumPayrollExcelColumn('net'),
        },
      );
                                ...
                                {(row.commResult.redeem.amount > 0 || row.commResult.sales.amount > 0 || row.commResult.sgm.amount > 0 || row.commResult.job > 0 || row.commResult.salesAmount.amount > 0 || row.commResult.salesBonus > 0 || row.commResult.payrollBonus > 0 || row.shopCommission > 0 || row.shopBonus > 0 || row.rawAnnualLeaveAverageCommissionPay > 0 || row.rawStatutoryHolidayAverageCommissionPay > 0) ? (
                                  <div className="grid gap-2 rounded-xl border border-slate-200 bg-slate-50 p-3 md:grid-cols-2 xl:grid-cols-4">
                                    ...
                                    {row.rawAnnualLeaveAverageCommissionPay > 0 ? (
                                      <div className="rounded-lg bg-white px-3 py-2">
                                        <div className="text-xs text-slate-500">AL 平均佣金 ({fmtDec(row.rollingAverageCommission)} × {row.annualLeaveDays}日){row.isPackageEmployee ? '（包佣後出數）' : ''}</div>
                                    ...
                                    {row.rawStatutoryHolidayAverageCommissionPay > 0 ? (
                                      <div className="rounded-lg bg-white px-3 py-2">
                                        <div className="text-xs text-slate-500">SH 平均佣金 ({fmtDec(row.rollingAverageCommission)} × {row.statutoryHolidayDays}日){row.isPackageEmployee ? '（包佣後出數）' : ''}</div>
            ...
            {(() => {
              ...
              const commissionIncomeRows: Array<[string, number | null]> = showPackageCommission
                ? [
                  ...
                  ...(activePayslipPdfEntry.shopCommission > 0 ? [[`Shop Commission  店舖佣金：大數 ${fmtPayslipAmount(activePayslipPdfEntry.shopActualSalesAmount)}`, activePayslipPdfEntry.shopCommission] as [string, number]] : []),
