...
export default function PayrollPage() {
  ...
  useEffect(() => { if (!user) return; Promise.all([fetchPayrollSummary(user), fetchCommissionRateTiers(), fetchMonthlyCommissionRecords(payrollMonth), fetchCommissionAverage365(), fetchPayrollBonusConfigCatalog(), fetchPayrollAttendanceRecords(user, payrollMonth), fetchPayrollSystemSettings(), fetchPayrollReviewAnswers(payrollMonth), fetchLatestMpfDeductionModesBeforeMonth(payrollMonth), fetchRollingCommissionAverages(user, payrollMonth)]).then(([employees, commissionTiers, savedRecords, commissionAvg, payrollBonusConfig, attendanceRecords, sys, payrollReviewAnswers, previousMpfDeductionModes, rollingCommissionAverages]) => { setData({ employees, commissionTiers, savedRecords, commissionAvg, payrollBonusConfig, attendanceRecords, defaultPackageNoPayHandling: sys.packageNoPayDefaultHandling, payrollReviewAnswers, previousMpfDeductionModes, rollingCommissionAverages }); }).catch(console.error); }, [payrollMonth, user]);
