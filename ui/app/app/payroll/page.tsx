...
export default function PayrollPage() {
  ...
  useEffect(() => { if (!user) return; let active = true; setData(null); Promise.all([fetchPayrollSummary(user), fetchCommissionRateTiers(), fetchMonthlyCommissionRecords(payrollMonth), fetchCommissionAverage365(), fetchPayrollBonusConfigCatalog(), fetchPayrollAttendanceRecords(user, payrollMonth), fetchPayrollSystemSettings(), fetchPayrollReviewAnswers(payrollMonth), fetchLatestMpfDeductionModesBeforeMonth(payrollMonth), fetchRollingCommissionAverages(user, payrollMonth)]).then(([employees, commissionTiers, savedRecords, commissionAvg, payrollBonusConfig, attendanceRecords, sys, payrollReviewAnswers, previousMpfDeductionModes, rollingCommissionAverages]) => { if (!active) return; setData({ month: payrollMonth, employees, commissionTiers, savedRecords, commissionAvg, payrollBonusConfig, attendanceRecords, defaultPackageNoPayHandling: sys.packageNoPayDefaultHandling, payrollReviewAnswers, previousMpfDeductionModes, rollingCommissionAverages }); }).catch((error) => { if (active) console.error(error); }); return () => { active = false; }; }, [payrollMonth, user]);
