...
import { describeAccessScope, normalizeAccessScope, serializeAccessScope } from '@/src/lib/auth/access';
...
type UserRow = {
  ...
  name: string | null;
  role: string;
  user_access_scope: unknown;
  app_access_scope: unknown;
...
function formatDate(value?: string | null) {
  ...
  return new Intl.DateTimeFormat('zh-HK', { year: 'numeric', month: 'short', day: 'numeric' }).format(new Date(value));
}
...
export default function ClientUserManagementPanel({
  companies,
  branches,
}: {
  ...
  const handleUpdateRole = async (e: React.FormEvent<HTMLFormElement>, userId: string) => {
    e.preventDefault();
    setSavingId(userId);
    setError(null);
    ...
    const role = String(fd.get('role') ?? 'employee');
    const allCompanies = fd.get('allCompanies') === 'true';
    const allBranches = fd.get('allBranches') === 'true';
    const companyIds = fd.getAll('companyIds').map(String).filter(Boolean);
    const branchIds = fd.getAll('branchIds').map(String).filter(Boolean);

    const accessScope = serializeAccessScope({ allCompanies, allBranches, companyIds, branchIds });
  ...
  if (loading) {
    return (
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <p className="text-sm text-slate-500">載入用戶資料中…</p>
      </section>
    );
  }

  const confirmedUsers = users.filter((u) => Boolean(u.email_confirmed_at)).length;
  const pendingUsers = users.length - confirmedUsers;
  const roleCount = new Set(users.map((u) => normalizeRole(u.role))).size;
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        ...
        <p className="text-sm font-medium text-slate-500">已載入 {users.length} 名用戶</p>
      </div>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[360px_minmax(0,1fr)]">
        <form onSubmit={handleCreateUser} className="space-y-4 rounded-xl border border-slate-200 bg-slate-50/70 p-4">
          <div>
          ...
          <button type="submit" disabled={creating} className="w-full rounded-xl bg-slate-900 px-4 py-3 text-sm font-semibold text-white transition-colors hover:bg-slate-800 disabled:opacity-50">
            {creating ? '建立中…' : '建立登入帳戶'}
          </button>
        </form>
              ...
              <tbody className="divide-y divide-slate-100 bg-white">
                {users.map((user) => {
                  ...
                  const currentScope = normalizeAccessScope(
                    user.app_access_scope ?? user.user_access_scope ?? null,
                    currentRole
                  );
                  ...
                  return (
                    <tr key={user.id} className="align-top hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="px-4 py-4">
                        <form onSubmit={(e) => handleUpdateRole(e, user.id)} className="space-y-3">
                          <div className="flex items-center gap-2">
                            ...
                            <button type="submit" disabled={isSaving} className="rounded-lg border border-slate-200 px-3 py-2 text-sm font-medium text-slate-700 transition-colors hover:bg-slate-50 disabled:opacity-50">
                              {isSaving ? '…' : '儲存'}
                            </button>
                          </div>
                      ...
                      <td className="px-4 py-4 text-right text-sm text-slate-500">{ROLE_LABELS[currentRole]}</td>
                    </tr>
                  );
                })}
              </tbody>
